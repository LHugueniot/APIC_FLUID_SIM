...
bool MacGrid::isBoundaryCell(int i, int j, int k) const{
	if ( i && (i < cellNum_i) && j && (j < cellNum_j) && (k && (k < cellNum_k)))
		return false;
	return true;
}
...
Vector3d MacGrid::cellCenterVel(int i, int j, int k) const{
	return {(double)(cellFaceVel_u[cellMinFaceIdx_u(i, j, k)] + cellFaceVel_u[cellMaxFaceIdx_u(i, j, k)] ) * .5f,
			(double)(cellFaceVel_v[cellMinFaceIdx_v(i, j, k)] + cellFaceVel_v[cellMaxFaceIdx_v(i, j, k)] ) * .5f,
			(double)(cellFaceVel_w[cellMinFaceIdx_w(i, j, k)] + cellFaceVel_w[cellMaxFaceIdx_w(i, j, k)] ) * .5f};
}
